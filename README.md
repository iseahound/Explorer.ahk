...
> By default checks only the current active window. To use the last active explorer window, pass `true` or `1`.
    ...
    MsgBox ExplorerPaths()  ; Gets all selected filepaths in the current explorer window

    ; Prints out the absolute filepaths of every selected file in the current explorer window
